...
private:
        ...
        void initTensorRT() {
                ...
                cudaMalloc(&gpu_buffers_[1], num_classes_ * output_h_ * output_w_ * sizeof(float));
                cudaMalloc(&gpu_buffers_[2], 2 * output_h_ * output_w_ * sizeof(float));
                cudaMalloc(&gpu_buffers_[3], 2 * output_h_ * output_w_ * sizeof(float));
        
                cpu_fmap_ = new float[num_classes_ * output_h_ * output_w_];
                cpu_reg_ = new float[2 * output_h_ * output_w_];  // 在CPU上分配cpu_reg_的内存(大小=2 * output_h_ * output_w_个float元素)
                cpu_wh_ = new float[2 * output_h_ * output_w_];  // 在CPU上分配cpu_wh_的内存(大小=2 * output_h_ * output_w_个float元素)
...
private:
    ...
    int previous_shoot_waypoint_idx_;  // 上一个射击任务点位索引
...
public:
        GoNode(int player_id) : Node("go_node"), player_id_(player_id), state_(MOVING),
                                                            ...
                                                            previous_shoot_waypoint_idx_(-1),  // 初始化上一个射击路径点索引为-1
...
private:
            ...
            precise_arrival_threshold = arrival_threshold_fast_;  // 设置快速到达的阈值(ARRIVAL_THRESHOLD_FAST=0.3米)
                ...
                if (distance <= arrival_threshold) {  // 将<赋值为arrival_threshold) {
                    ...
                    // 检查是否在被攻击状态下到达新的射击任务点位
                                        if (is_under_attack_ && previous_shoot_waypoint_idx_ >= 0 && waypoint_idx != previous_shoot_waypoint_idx_) {  // 如果条件满足,执行对应操作
                        ...
                        // 只有当自身血量仍然减少时才返回上一点位
                                                if (self_health_change < 0) {
                                                        RCLCPP_INFO(this->get_logger(), "P%zu: 被攻击状态下到达，自身血量仍在减少(%d)，返回上一点位P%d",  // 输出信息日志
                                                                                                                                    waypoint_idx + 1, self_health_change, previous_shoot_waypoint_idx_ + 1);
                            // 返回上一点位继续任务
                                                        moveToWaypoint(previous_shoot_waypoint_idx_, current_time);  // 移动到上一个射击路径点
                                                        is_under_attack_ = false;  // 将is_under_attack_设置为false
                                                        return;  // 返回函数(退出函数,不返回值)
                                                } else {  // 否则(后面的路径点,精确到达)
                            // 自身血量不再减少，清除被攻击状态
                                                        is_under_attack_ = false;
                                                        previous_shoot_waypoint_idx_ = -1;  // 将previous_shoot_waypoint_idx_赋值为-1
        ...
        void healthCheckLoop() {
                ...
                if (enemy_health_valid && current_enemy_health <= 0) {  // 如果敌人血量有效且当前血量小于等于0(敌人已被击杀,处理击杀逻辑)
                        ...
                        state_ = MOVING;
                        is_under_attack_ = false;
                        previous_shoot_waypoint_idx_ = -1;
        ...
        // 保存当前射击任务点位索引
                previous_shoot_waypoint_idx_ = waypoint_idx;  // 将previous_shoot_waypoint_idx_赋值为waypoint_idx
